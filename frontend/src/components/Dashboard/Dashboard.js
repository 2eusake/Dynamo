...
const Dashboard = () => {
  ...
  const projectPerformanceData = projects.map(project => ({
    name: project.name,
    actualTime: tasks.filter(task => task.projectId === project.id)
      .reduce((sum, task) => sum + task.actualHours, 0),
    allocatedTime: tasks.filter(task => task.projectId === project.id)
      .reduce((sum, task) => sum + task.allocatedHours, 0),
  }));
      ...
      <main className="flex-1 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={projectPerformanceData}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="actualTime" fill={colors.primary} name="Actual Time" />
                  <Bar dataKey="allocatedTime" fill={colors.secondary} name="Allocated Time" />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              {/* Implement resource utilization chart here */}
              <p>Resource utilization chart to be implemented</p>
            </CardContent>
          </Card>
        </div>
      </main>
